...
namespace ScriptLab {
  ...
  export function getAccessToken(clientId?: string, resource?: string): Promise<string> {
    ...
    if (cachedAccessToken) {
      ...
      throw new Error(
        (ScriptLab as any)._strings.officeVersionDoesNotSupportAuthentication
      );
    }
  }
  ...
  function _getAccessTokenViaWindowOpen(
    clientId: string,
    resource: string
  ): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      const dialogCloseWatcher = setInterval(() => {
        if (!authDialog || authDialog.closed) {
          ...
          reject(
            new Error((ScriptLab as any)._strings.authenticationWasCancelledByTheUser)
          );
        }
      }, 1000);
  ...
  function _getAccessTokenViaDialogApi(
    clientId: string,
    resource: string
  ): Promise<string> {
    return new Promise((resolve, reject) => {
          ...
          dialog.addEventHandler(Office.EventType.DialogEventReceived, event => {
            switch (event.error) {
              case CODE_DIALOG_CLOSED_BY_USER:
                reject(
                  new Error(
                    (ScriptLab as any)._strings.authenticationWasCancelledByTheUser
                  )
                );
                return;
              default:
          ...
          dialog.addEventHandler(Office.EventType.DialogMessageReceived, args => {
            try {
              ...
              try {
                ...
                if (message.type === 'auth') {
                  resolve(
                    _extractAndCacheAccessToken(message.message, clientId, resource)
                  );
                  dialog.close();
                }
              } catch (exception) {}
  ...
  function _isInsideOffice() {
    return window && (window as any).Office && (window as any).Office.context;
  }
  ...
  function _extractAndCacheAccessToken(
    message: any,
    clientId: string,
    resource: string
  ): string {
    cachedAuthTokens[_cacheKey(clientId, resource)] = {
      ...
      expiry:
        Date.now() +
        (message.expiresIn - 60 * 5) /* five minute of safety margin */ * 1000,
    };
