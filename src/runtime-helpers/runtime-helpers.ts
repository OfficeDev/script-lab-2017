...
module ScriptLab {
    ...
    */
    ...
    /** [PREVIEW] Opening files with the OneDrive File Picker
     * @param clientId: The application ID generated by the app registration console for your snippet
    */
    export function filePickerOpen(clientId: string): Promise<string> {
        if (_isPlainWeb()) {
            return _filePickerViaWindowOpen(clientId);
        }
        if (_isDialogApiSupported()) {
            return _filePickerViaDialogApi(clientId);
        }
        throw new Error((ScriptLab as any)._strings.officeVersionDoesNotSupportFilePicker);
    }

    function _filePickerViaWindowOpen(clientId: string): Promise<string> {
        return new Promise((resolve, reject) => {

            let filePickerDialog = window.open(
                _generateFilePickerUrl({ filePicker_action: 'open', client_id: clientId, is_office_host: false }),
                '_blank',
                'width=1024,height=768'
            );
            window.addEventListener('message', filePickerMessageListener);

            const dialogCloseWatcher = setInterval(() => {
                if (!filePickerDialog || filePickerDialog.closed) {
                    clearInterval(dialogCloseWatcher);
                    reject(new Error((ScriptLab as any)._strings.authenticationWasCancelledByTheUser));
                }
            }, 1000);

            function filePickerMessageListener(event: MessageEvent) {
                if (event.origin !== window.location.origin) {
                    return;
                }
                if (typeof event.data !== 'string') {
                ...
                try {
                    ...
                    if (message.type === 'files') {
                        window.removeEventListener('message', filePickerMessageListener);
                ...
                if (event.data.indexOf('FILEPICKER:error=') === 0) {
                    window.removeEventListener('message', filePickerMessageListener);
                    reject(new Error(event.data.substr('FILEPICKER:error='.length)));
                    filePickerDialog.close();
                    return;
                }
            }
    ...
    function _filePickerViaDialogApi(clientId: string): Promise<string> {
        return new Promise((resolve, reject) => {
            Office.context.ui.displayDialogAsync(
                _generateFilePickerUrl({ filePicker_action: 'open', client_id: clientId, is_office_host: false }),
                { height: 50, width: 50 },
                result => {
                    ...
                    dialog.addEventHandler(Office.EventType.DialogMessageReceived, args => {
                        try {
                            ...
                            if (args.message.indexOf('FILEPICKER:error=') === 0) {
                                reject(new Error(args.message.substr('FILEPICKER:error='.length)));
                                dialog.close();
                            }
                        }
                        catch (exception) {
    ...
    function _generateFilePickerUrl(params: {
        filePicker_action: string;
        client_id: string;
        is_office_host: boolean;
    }): string {
        const queryParams = [
            `filePicker_action=${params.filePicker_action}`,
            `client_id=${encodeURIComponent(params.client_id)}`,
            `is_office_host=${params.is_office_host}`
        ].join('&');
        return window.location.origin + '/snippet/filepicker?' + queryParams;
    }
}
