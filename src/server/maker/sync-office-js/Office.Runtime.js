...
(function (OfficeExtension) {
    ...
    var ClientRequestContext = (function () {
        ...
        ClientRequestContext.prototype.syncPrivatePreNetworkRoundtrip = function (req) {
            ...
            return { requestFlags: requestFlags, requestExecutorRequestMessage: requestExecutorRequestMessage };
        };
        ClientRequestContext.prototype.syncPrivate = function (req) {
            ...
            var _a = this.syncPrivatePreNetworkRoundtrip(req), requestFlags = _a.requestFlags, requestExecutorRequestMessage = _a.requestExecutorRequestMessage;
        ...
        ClientRequestContext.prototype.syncSynchronous = function () {
            var req = this._pendingRequest;
            this.m_pendingRequest = null;
            var _a = this.syncPrivatePreNetworkRoundtrip(req), requestFlags = _a.requestFlags, requestExecutorRequestMessage = _a.requestExecutorRequestMessage;
            var response = OfficeExtension.SynchronousHttpRequestExecutor.executeSynchronously(this._customData, requestFlags, requestExecutorRequestMessage);
            var errorFromResponse = this.processRequestExecutorResponseMessage(req, response);
            if (errorFromResponse) {
                OfficeExtension.Utility.log("Throw error from response: " + JSON.stringify(errorFromResponse));
                throw errorFromResponse;
            }
...
(function (OfficeExtension) {
    var HttpUtility = (function () {
        ...
        HttpUtility.xhrSendRequestFuncSynchronous = function (request) {
            var xhr = new XMLHttpRequest();
        ...
        HttpUtility.sendRequestSync = function (request) {
            HttpUtility.validateAndNormalizeRequest(request);
            return HttpUtility.xhrSendRequestFuncSynchronous(request);
        };
...
var OfficeExtension;
(function (OfficeExtension) {
    var SynchronousHttpRequestExecutor = (function () {
        function SynchronousHttpRequestExecutor() {
        }
        SynchronousHttpRequestExecutor.executeSynchronously = function (customData, requestFlags, requestMessage) {
            var requestMessageText = JSON.stringify(requestMessage.Body);
            var url = requestMessage.Url;
            if (url.charAt(url.length - 1) != "/") {
                url = url + "/";
            ...
            requestInfo.headers[OfficeExtension.Constants.sourceLibHeader] = SynchronousHttpRequestExecutor.SourceLibHeaderValue;
            requestInfo.headers["CONTENT-TYPE"] = "application/json";
            if (requestMessage.Headers) {
                for (var key in requestMessage.Headers) {
                    requestInfo.headers[key] = requestMessage.Headers[key];
            ...
            var responseInfo = OfficeExtension.HttpUtility.sendRequestSync(requestInfo);
            var response;
            if (responseInfo.statusCode === 200) {
                response = {
                    ErrorCode: null,
                    ErrorMessage: null,
                    Headers: responseInfo.headers,
                    Body: JSON.parse(responseInfo.body)
                };
            }
            else {
                OfficeExtension.Utility.log("Error Response:" + responseInfo.body);
        ...
        SynchronousHttpRequestExecutor.SourceLibHeaderValue = "officejs-rest-sync";
        return SynchronousHttpRequestExecutor;
    }());
    OfficeExtension.SynchronousHttpRequestExecutor = SynchronousHttpRequestExecutor;
})(OfficeExtension || (OfficeExtension = {}));
var OfficeExtension;
(function (OfficeExtension) {
