...
export function injectPerfMarkers(input: string): string {
    const sourceFile = ts.createSourceFile('usercode', input, ts.ScriptTarget.ES2015, true /*setParentNodes */);
        ...
        switch (node.kind) {
            ...
            case ts.SyntaxKind.VariableStatement:
                ...
                if (node.parent && node.parent.kind === ts.SyntaxKind.Block) {
                    ...
                    const node_line_no = sourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1;
