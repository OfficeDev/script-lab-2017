...
(() => {
  ...
  (window as any).initializeRunner = async (
    ...
    try {
      ...
      document.getElementById(
        'choose-your-host'
      ).textContent = Strings().HtmlPageStrings.chooseYourHost;
  ...
  async function initializeRunnerHelper(initialParams: InitializationParams) {
    ...
    if (initialParams.explicitlySetDisplayLanguageOrNull) {
      ...
      document.cookie = `displayLanguage=${encodeURIComponent(
        initialParams.explicitlySetDisplayLanguageOrNull
      )};path=/;`;
    }
    ...
    let isTrustedSnippet = isNil(initialParams.isTrustedSnippet)
      ? false
      : initialParams.isTrustedSnippet;
    ...
    let runnerUrlWithCorrectPrefix = (() => {
      let urlExtractor = /^(http[s]?:)\/\/(.*)$/gi.exec(
  ...
  function replaceSnippetIframe({
    ...
    (window as any).scriptRunnerBeginInit = (
      iframeWindow: Window,
      options: { scriptReferences: string[] } = { scriptReferences: [] }
    ) => {
    ...
    contentWindow.onerror = (...args) => {
      ...
      UI.notify(
        Strings().Runner.runtimeErrorWhileLoadingTheSnippet,
  ...
  function onPerfAnalysisReady(perf: PerfInfoItem[]) {
    heartbeat.messenger.send<{ perf: PerfInfoItem[] }>(
      heartbeat.window,
      RunnerMessageType.SNIPPET_PERF_DATA,
      { perf: perf }
    );
  }
  ...
  function establishHeartbeat(origin: string, heartbeatParams: HeartbeatParams) {
    ...
    heartbeat.messenger
      ...
      .subscribe(input => {
        if (isListeningToCurrentSnippetContentChange) {
          showReloadNotification(
            $('#notify-current-snippet-changed'),
            () =>
              clearAndRefresh(
                currentSnippet.id,
                input.message.name,
                false /*isTrustedSnippet*/
              ),
    ...
    heartbeat.messenger
      ...
      .subscribe(input => {
        ...
        if (input.message.id === currentSnippet.id) {
          ...
          $anotherSnippetSelected
            .find('.ms-MessageBar-text .snippet-name')
            .text(input.message.name);
          showReloadNotification(
            $anotherSnippetSelected,
            () =>
              clearAndRefresh(
    ...
    heartbeat.messenger
      .listen<{ snippet: ISnippet; isTrustedSnippet: boolean }>()
      ...
      .subscribe(input => {
        ...
        $.post(window.location.origin + '/compile/snippet', {
          data: data,
          isTrustedSnippet: input.message.isTrustedSnippet,
        })
          .then(html =>
            processSnippetReload(html, snippet, input.message.isTrustedSnippet)
          )
          .fail(handleError);
      });
  }
  ...
  function toggleProgressLoadingIndicators(visible: boolean) {
    $('#progress .cs-loader, #subtitle').css(
      'visibility',
  ...
  function initializeTooltipUpdater() {
    ...
    function refreshLastUpdatedText() {
      ...
      const momentText = moment(currentSnippet.lastModified)
        .locale(getDisplayLanguageOrFake())
        .fromNow();
      $headerTitle.attr(
        'title',
        `${strings.HtmlPageStrings.lastUpdated} ${momentText}. ${
          strings.HtmlPageStrings.clickToRefresh
        }.`
      );
    }
  }
