...
tryCatch(async () => {
  ...
  let authRequestParams: AuthRequestParamData =
  ...
  if (
    !hasActionAndServiceAndClientIdInfo &&
    window.sessionStorage[AuthRequestSessionStorageKey]
  ) {
  ...
  if (
    !authRequestParams.auth_action ||
    !authRequestParams.client_id ||
    !authRequestParams.resource
  ) {
...
function proceedWithAuthInit(authRequest: AuthRequestParamData) {
  tryCatch(() => {
    ...
    if (authRequest.auth_action === 'login') {
      ...
      if (hasHash) {
        const authResponseKeyValues = Authenticator.extractParams(
        ...
        if (accessToken) {
          ...
          if (authRequest.is_office_host) {
            ...
            if (window.parent) {
              ...
              const accessTokenInputBox = document.getElementById(
                'access-token-if-no-redirect'
              ) as HTMLInputElement;
          ...
          throw new Error(
            authResponseKeyValues['error'] +
              ': ' +
              authResponseKeyValues['error_description']
          );
        } else {
      ...
      if (authRequest[ResourceParamName] && authRequest[ClientIdParamName]) {
        ...
        if (!resource) {
          throw new Error(
            `${strings.Auth.unrecognizedResource} "${authRequest.resource}"`
          );
        }
        ...
        if (isDefaultClientId) {
          const intermediaryUrl = generateUrl(
            `${environment.current.config.editorUrl}/default-auth.html`,
            {
...
function hideProgressFooter() {
  ...
  )[0] as HTMLDivElement).style.visibility =
    'hidden';
}
