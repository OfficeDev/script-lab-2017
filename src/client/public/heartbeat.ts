...
(() => {
  ...
  tryCatch(() => {
    const params: HeartbeatParams = Authenticator.extractParams(
    ...
    let previousLastOpenedIdAndTimestamp: string = lastOpened
      ? `${lastOpened.id}.${lastOpened.modified_at}`
      : '';

    setInterval(() => {
      ensureFreshLocalStorage();
      const currentLastOpened = getLastOpenedSnippet();
      let currentLastOpenedIdAndTimestamp: string = currentLastOpened
        ? `${currentLastOpened.id}.${currentLastOpened.modified_at}`
        : '';
  ...
  function getLastOpenedSnippet(): ISnippet | null {
    let settings: ISettings = (JSON.parse(
      window.localStorage.getItem(localStorageKeys.settings)
  ...
  function getSnippetById(id: string): ISnippet | null {
    ...
    const snippets: { [key: string]: ISnippet } =
  ...
  function sendBackCurrentSnippet(isTrustedSnippet: boolean) {
    ...
    if (trackingSnippet.id) {
      ...
      if (lastOpened) {
        ...
        messenger.send(
          window.parent,
          RunnerMessageType.ERROR,
          Strings().Runner.noSnippetIsCurrentlyOpened
        );
        return;
      }
    }
    ...
    if (snippet == null) {
      ...
      messenger.send(
        window.parent,
        RunnerMessageType.ERROR,
        Strings().Runner.snippetNoLongerExists
      );
      return;
    }
    ...
    if (snippet.modified_at !== trackingSnippet.lastModified) {
      ...
      if (sendImmediately) {
        ...
        isTrustedSnippet = trustedSnippetManager.isSnippetTrusted(
          snippet.id,
          snippet.gist,
          snippet.gistOwnerId
        );
        messenger.send(window.parent, RunnerMessageType.REFRESH_RESPONSE, {
          snippet: snippet,
  ...
  function setupRequestReloadListener(messenger: Messenger<RunnerMessageType>) {
    messenger
      .listen<{ id: string; isTrustedSnippet: boolean }>()
