...
interface RunnerCustomFunctionMetadata
  extends ICustomFunctionsSnippetRegistrationData {
  id: string;
}
...
(async () => {
  ...
  try {
    ...
    heartbeat.messenger.send<{ timestamp: number }>(
      heartbeat.window,
      CustomFunctionsMessageType.LOADED_AND_RUNNING,
  ...
  return new Promise(resolve => {
    ...
    (window as any).scriptRunnerEndInit = (
      ...
      snippetMetadata.functions.map(func => {
        ...
        iframeWindow['console']['error'] = consoleMsgTypeImplementation(
          'error'
        );
          ...
          return function(...args) {
            ...
            args.forEach((element, index, array) => {
              try {
                ...
                logMsg +=
                  'Error on console logging this argument ' +
                  e.toString() +
                  ', ';
              }
            });
