...
export function getChineseSimplifiedStrings(): ClientStringsPerLanguage {
  ...
  return {
    ...
    snippetNoOfficeMessage:
      '只能在Office插件中运行office代码段。获取脚本。 现在免费获取 Script Lab 请登入网站 https://aka.ms/getscriptlab。',
    ...
    snippetExportNotSupported:
      '此版本的Office还不支持代码段导出. 支持的平台包括Windows和Office在线。',
    ...
    tabDisplayNames: {
      script: '脚本',
      template: getEnglishSubstitutesForNotYetTranslated().tabDisplayNames
        .customFunctions,
    ...
    noSnippetsMessage:
      '没有本地代码段. 你可以创建一个新的，选择一个样本，或从其他地方输入代码段。',
