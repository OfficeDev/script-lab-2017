...
export class About implements AfterViewInit {
  ...
  private _hostSnippetsStorageKey = localStorageKeys.hostSnippets_parameterized.replace(
    '{0}',
  ...
  cache = [
    ...
    `${storageSize(
      sessionStorage,
      sessionStorageKeys.intelliSenseCache,
      Strings().aboutIntellisense
    )}`,
  ].join('\n');
  ...
  ngAfterViewInit() {
    ...
    const isLocalHost =
      environment.current.config.name === config.local.name ||
      /localhost/.test(
    ...
    this.selectedConfig = this.configs.find(
      c => c.value.toUpperCase() === environment.current.config.name
    ).value;

    this.experimentationFlags = environment.getExperimentationFlagsString(
      true /*onEmptyReturnDefaults*/
    );
    const isEffectivelyEmpty = isEqual(
      JSON.parse(this.experimentationFlags),
  ...
  async okClicked() {
    ...
    let experimentationUpdateResultOrError = attempt(() =>
      environment.updateExperimentationFlags(this.experimentationFlags)
    );

    if (isError(experimentationUpdateResultOrError)) {
      await this._effects.alert(
        experimentationUpdateResultOrError.message,
        this.strings.error,
        this.strings.ok
      );
      return;
  ...
  logoutSnippets() {
    window.open(
      _generateAuthUrl({
        client_id: environment.current.config.thirdPartyAADAppClientId,
        resource: 'graph',
      }),
      '_blank',
    ...
    function _generateAuthUrl(params: { resource: string; client_id: string }): string {
      const queryParams = [
        `auth_action=logout`,
  ...
  onExperimentationFlagsChange() {
    if (this.experimentationFlags.trim().length === 0) {
      this.experimentationFlags = JSON.stringify(
        PLAYGROUND.experimentationFlagsDefaults,
        null,
        4
      );
  ...
  async _handleEnvironmentSwitching() {
    ...
    ensureFreshLocalStorage();
    let originEnvironment = window.localStorage.getItem(
