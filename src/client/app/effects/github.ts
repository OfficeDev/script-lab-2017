...
export class GitHubEffects {
  ...
  logout$: Observable<Action> = this.actions$
    ...
    .catch(exception =>
      Observable.of(new UI.ReportErrorAction(Strings().githubLogoutFailed, exception))
    );
  ...
  isLoggedIn$: Observable<Action> = this.actions$
    ...
    .catch(exception =>
      Observable.of(new UI.ReportErrorAction(Strings().profileCheckFailed, exception))
    );
  ...
  loadGists$: Observable<Action> = this.actions$
    ...
    .map(gists => {
      return gists
        .map(gist => ({
          ...
          file: find(
            gist.files,
            (file, name) => (file ? /\.ya?ml$/gi.test(name) : false)
          ),
        }))
    ...
    .catch(exception =>
      Observable.of(new UI.ReportErrorAction(Strings().gistRetrieveFailed, exception))
    );
    ...
    .mergeMap(action => {
      ...
      return this._github
          ...
          if (!gistOwnerId && exception.status >= 400 && exception.status <= 499) {
            throw new PlaygroundError(
              JSON.stringify({
                type: type,
                snippetId: id,
                gistOwnerId: UNKNOWN_GIST_OWNER_ID,
              })
            );
          }
          throw exception;
        ...
        .mergeMap(async (item: { type: any; gist: IGist; snippetId: string }) => {
          ...
          let messageTitle =
            type === GitHub.GitHubActionTypes.UPDATE_GIST
              ? Strings().gistUpdateSuccess
              : Strings().gistSharedDialogTitle;

          let result = await this._uiEffects.alert(
            messageBody,
            messageTitle,
            Strings().gistSharedDialogViewButton,
            Strings().ok
          ); // the URL should be a hyperlink and the text should wrap
        ...
        .mergeMap(({ gist, snippetId }) =>
          Observable.from([
            new GitHub.LoadGistsAction(),
            new GitHub.ShareSuccessAction(gist),
            new Snippet.UpdateInfoAction({
              id: snippetId,
              gist: gist.id,
              gistOwnerId: gist.owner.login,
            }),
          ])
        )
        .catch(exception => {
          ...
          if (exception instanceof PlaygroundError) {
            try {
              ...
              if (message.type === GitHub.GitHubActionTypes.UPDATE_GIST) {
                actions.push(
                  new Snippet.UpdateInfoAction({
                    id: message.snippetId,
                    gistOwnerId: message.gistOwnerId,
                  })
                );
              }
            } catch (e) {
  ...
  shareCopy$: Observable<Action | void> = this.actions$
    ...
    .map((rawSnippet: ISnippet) => {
      ...
      new clipboard('#CopyToClipboard', {
        ...
        await this._uiEffects.alert(
          Strings().snippetCopiedConfirmation,
          null,
          Strings().ok
        );
  ...
  shareExport$: Observable<Action> = this.actions$
    ...
    .map((snippet: ISnippet) => {
      if (
        ...
        Utilities.platform === PlatformType.IOS
      ) {
        AI.trackEvent('Unsupported share export', { id: snippet.id });
        this._store.dispatch(
          this._createShowErrorAction(Strings().snippetExportNotSupported, null)
        );
        return null;
      }
