...
export class SnippetEffects {
  ...
  save$: Observable<Action> = this.actions$
    ...
    .map(rawSnippet => {
      ...
      if (storage.snippets.contains(scrubbedSnippet.id)) {
        ...
        const originalScrubbedSnippet = getScrubbedSnippet(
          originalRawSnippet,
          publicOrInternal
        );
    ...
    .map(scrubbedSnippet => {
      ...
      const hostStorageKey = localStorageKeys.hostSnippets_parameterized.replace(
        '{0}',
  ...
  duplicate$: Observable<Action> = this.actions$
    ...
    .map(id => {
      const original = getScrubbedSnippet(
        storage.snippets.get(id),
        SnippetFieldType.PUBLIC
      );
  ...
  loadSnippets$: Observable<Action> = this.actions$
    .ofType(
      Snippet.SnippetActionTypes.STORE_UPDATED,
      Snippet.SnippetActionTypes.LOAD_SNIPPETS
    )
  ...
  run$: Observable<Action | void> = this.actions$
    ...
    .map((snippet: ISnippet) => {
      if (isCustomFunctionScript(snippet.script.content)) {
        ...
        post(environment.current.config.runnerUrl + '/compile/page', {
          data,
          isTrustedSnippet,
        });
      }
  ...
  private _getSnippetsWithMatchingGistID(id: string): ISnippet[] {
    return storage.snippets
      .values()
      .filter(item => item.gist && item.gist.trim() === id.trim());
  }
   ...
   */
  private _updateSnippetNameIfNeeded(
    snippet: ISnippet,
    mode: string,
  ...
  private _generateName(name: string, suffix: string = ''): string {
    ...
    let collisions = storage.snippets
      .values()
      .filter(item => regex.test(item.name.trim()));
    let maxSuffixNumber = reduce(
      collisions,
      (max, item: any) => {
    ...
    return `${newName}${suffix ? ' - ' + suffix : ''}${
      maxSuffixNumber ? ' - ' + maxSuffixNumber : ''
    }`;
  }
  ...
  private _importRawFromSource(data: string, type: string): Observable<ISnippet> {
    ...
    switch (type) {
      ...
      case Snippet.ImportType.GIST:
        ...
        const match = /https:\/\/gist.github.com\/(?:.*?\/|.*?)([a-z0-9]{32})$/.exec(
          data
        );
        ...
        if (match) {
          ...
          if (data.length === 32 && !/https?:\/\//.test(data)) {
            ...
            return this._request.get<ISnippet>(
              data,
              ResponseTypes.YAML,
              true /*force bypass of cache*/
            );
          }
        }

        /* use the github api to get the gist, needed for secret gists as well */
        return this._github.gist(id).map(gist => {
          /* Try to find a yaml file */
          let snippet = find(
            gist.files,
            (value, key) => (value ? /\.ya?ml$/gi.test(key) : false)
          );
  ...
  private async _massageSnippet(
    ...
    if (rawSnippet.host && rawSnippet.host !== environment.current.host) {
      throw new PlaygroundError(
        Strings().cannotImportSnippetCreatedForDifferentHost(
          rawSnippet.host,
          environment.current.host
        )
      );
    ...
    if (mode !== Snippet.ImportType.OPEN && snippet.gist) {
      ...
      if (snippetsWithSameGistId.length > 0) {
        let options = [
          ...
          snippetsWithSameGistId.length === 1
            ? Strings().overwriteExistingButtonLabel
            : null,
    ...
    let getActionsFunc = (): Action[] => {
      switch (importResult) {
        ...
        default: {
          this._updateSnippetNameIfNeeded(
            snippet,
            mode,
  ...
  private _checkForUnsupportedAPIsIfRelevant(
    api_set: { [index: string]: number },
    snippet: ISnippet
  ) {
    ...
    const desiredOfficeJS =
      processLibraries(
        snippet.libraries,
        isMakerScript(snippet.script),
        isInsideOfficeApp()
      ).officeJS || '';
    if (
      desiredOfficeJS
        .toLowerCase()
        .indexOf('https://appsforoffice.microsoft.com/lib/1/hosted/') < 0
    ) {
    ...
    if (unsupportedApiSet) {
      throw new PlaygroundError(
        Strings().currentHostDoesNotSupportRequiredApiSet(
          environment.current.host,
          unsupportedApiSet.api,
          unsupportedApiSet.version.toString()
        )
      );
    }
  }
