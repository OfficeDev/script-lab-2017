...
export class MonacoEffects {
  ...
  constructor(private actions$: Actions, private _request: Request) {
  ...
  updateIntellisense$: Observable<Action> = this.actions$
    ...
    .mergeMap(({ files, language }) =>
      Observable.from(files).map(
        file => new Monaco.AddIntellisenseAction({ url: file, language })
      )
    )
    .catch(exception =>
      Observable.of(
        new UI.ReportErrorAction(Strings().intellisenseUpdateError, exception)
      )
    );
  ...
  addIntellisense$: Observable<Action> = this.actions$
    ...
    .mergeMap((action: Monaco.AddIntellisenseAction) =>
      this._addIntellisense(action.payload)
    )
    .map(() => new Monaco.UpdateIntellisenseSuccessAction())
    .catch(exception =>
      Observable.of(new UI.ReportErrorAction(Strings().intellisenseClearError, exception))
    );
  ...
  private _addIntellisense(payload: { url: string; language: string }) {
    ...
    return Observable.fromPromise(MonacoService.current)
      ...
      .map(file => {
        AI.trackEvent(Monaco.MonacoActionTypes.ADD_INTELLISENSE, {
          library: sha1(file.url).toString(),
        });
      ...
      .catch(exception =>
        Observable.of(
          new UI.ReportErrorAction(Strings().intellisenseLoadError, exception)
        )
      );
  }
    ...
    if (storage.intellisenseCache.contains(url)) {
      ...
      return this._request
        .get<string>(
          ...
          ResponseTypes.TEXT,
          null,
          environment.current.runtimeSessionTimestamp
        )
