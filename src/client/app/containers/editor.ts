...
export class Editor implements AfterViewInit {
  ...
  constructor(private _store: Store<fromRoot.State>, private _monaco: MonacoService) {
  ...
  updateIntellisense() {
    ...
    this._store.dispatch(
      new Monaco.UpdateIntellisenseAction({
  ...
  startPerfInfoTimer() {
    ...
    this.perfInfoPoller = setInterval(() => {
      ensureFreshLocalStorage();
      const newPerfNums = Number(
        window.localStorage.getItem(localStorageKeys.lastPerfNumbersTimestamp)
      );
  ...
  setPerformanceMarkers(perfInfo: PerfInfoItem[]) {
    ...
    this.currentDecorations = this._monacoEditor.deltaDecorations(
  ...
  setCodeLensPerfNumbers(perfInfo: PerfInfoItem[]) {
    ...
    this.currentCodeLensProvider = monaco.languages.registerCodeLensProvider(
      'typescript',
      {
              ...
              command: {
                ...
                title:
                  frequency === 1
                    ...
                    : `Ran ${frequency} times. Total Duration: ${duration} ms.`,
              },
            };
          });
        },
  ...
  private _subscribeToState() {
    this.menuSub = this._store.select(fromRoot.getMenu).subscribe(() => this._resize());

    this.themeSub = this._store
    ...
    this.tabSub = this._store.select(fromRoot.getActiveTab).subscribe(newTab => {
      if (newTab == null) {
        // RESET Action
      ...
      if (newTab) {
        ...
        let timer = AI.trackPageView(
          this.currentState.displayName,
          `/edit/${this.currentState.name}`
        );
  ...
  private async _changeSnippet(snippet) {
    ...
    this.isSnippetTrusted = trustedSnippetManager.isSnippetTrusted(
      this.snippet.id,
      this.snippet.gist,
      this.snippet.gistOwnerId
    );
    this.clearPerformanceMakers();
    this.changeTab();
  }
