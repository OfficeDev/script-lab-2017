...
export class EditorMode {
  ...
  get isCustomFunctionsSnippet() {
    return (
      this.snippet &&
      this.snippet.script &&
      isCustomFunctionScript(this.snippet.script.content)
    );
  }
  ...
  get isGistOwned() {
    return this.profile$
      ...
      .map(profile => {
        ...
        return isEmpty(this.snippet.gistOwnerId)
          ? true
          : this.snippet.gistOwnerId === profile.login;
      });
  }
  ...
  async delete() {
    ...
    let result = await this._effects.alert(
      this.strings.deleteSnippetConfirm,
      `${this.strings.delete} ${this.snippet.name}`,
      this.strings.delete,
      this.strings.cancel
    );
  ...
  shareGist(values) {
    ...
    let sub = this.isLoggedIn$.subscribe(async isLoggedIn => {
      if (!isLoggedIn) {
        this._store.dispatch(new GitHub.LoginAction());
        return;
      }
      ...
      if (isUpdate) {
        ...
        if (isGistOwned && this.snippet.gist) {
          confirmationAlertIfAny = await this._effects.alert(
            this.strings.sharePublicSnippetConfirm,
            `${this.strings.share} ${this.snippet.name}`,
            this.strings.share,
            this.strings.cancel
          );
        }
        ...
        if (isGistOwned && this.snippet.gist) {
          confirmationAlertIfAny = await this._effects.alert(
            this.strings.sharePrivateSnippetConfirm,
            `${this.strings.share} ${this.snippet.name}`,
            this.strings.share,
            this.strings.cancel
          );
  ...
  showErrors() {
    this.errorsSub = this.errors$
      ...
      .subscribe(errors => {
        ...
        this._effects
          .alert(data, this.strings.errors, this.strings.dismiss)
  ...
  parseEditorRoutingParams() {
    let sub = this._route.params
      ...
      .mergeMap(({ type, host, id }): Observable<{
        valid: boolean;
        mode: string;
        id: string;
      }> => {
  ...
  _processInitializationImport(params: {
    valid: boolean;
    mode: string;
    id: string;
  }): void {
    ...
    const postImportCompleteMessageIfRelevant = (snippetId: string | null) => {
      if (this.isEditorTryIt) {
        window.parent.postMessage(
          { type: 'import-complete', id: snippetId },
    ...
    switch (mode) {
      ...
      case Snippet.ImportType.GIST:
        this._store.dispatch(
          new Snippet.ImportAction({
            ...commonImportActionParams,
            saveToLocalStorage: !this.isEditorTryIt,
            data: id,
          })
        );
        break;
