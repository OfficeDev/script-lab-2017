...
export class GitHubService {
  ...
  set profile(value) {
    this._profile = value;
    storage.appendCurrent({ profile: value });
  }

  user(): Observable<IBasicProfile> {
    return this._request.get<IBasicProfile>(
      `${this._baseUrl}/user`,
      ResponseTypes.JSON,
      this._headers
    );
  ...
  orgs(user: string): Observable<IExtendedProfile[]> {
    return this._request.get<IExtendedProfile[]>(
      `${this._baseUrl}/users/${user}/orgs`,
      ResponseTypes.JSON,
      this._headers
    );
  }
  ...
  repos(org: string, personal: boolean, page: number = 0): Observable<IRepository[]> {
    let url = personal
      ? `${
          this._baseUrl
        }/user/repos?page=${page}&affiliation=owner,collaborator&sort=updated&direction=desc`
  ...
  files(
    ...
    branch: string,
    path?: string
  ): Observable<IContents[]> {
    let url = `${this._baseUrl}/repos/${org}}/${repo}/contents`;
    if (!(path == null)) {
      url += `/${path}`;
    }
    return this._request.get<IContents[]>(
      url + `?ref=${branch}`,
      ResponseTypes.JSON,
      this._headers
    );
  ...
  branches(org: string, repo: string): Observable<IBranch[]> {
    return this._request.get<IBranch[]>(
      `${this._baseUrl}/repos/${org}/${repo}/branches`,
      ResponseTypes.JSON,
      this._headers
    );
  }
  ...
  file(org: string, repo: string, branch: string, file: string): Observable<IContents> {
    return this._request.get<IContents>(
      `${this._baseUrl}/repos/${org}/${repo}/contents/${file}?ref=${branch}`,
      ResponseTypes.JSON,
      this._headers
    );
  }
  ...
  commits(
    ...
    return this._request.get<ICommit[]>(
      `${
        this._baseUrl
      }/repos/${org}/${repo}/commits?path=${file}&sha=${branch}&until=${new Date().toISOString()}`,
      ResponseTypes.JSON,
      this._headers
    );
  }
  ...
  createOrUpdate(
    ...
    file: string,
    body: any
  ): Observable<IUploadCommit> {
    return this._request.put<IUploadCommit>(
      `${this._baseUrl}/repos/${org}/${repo}/contents/${file}`,
      body,
      ResponseTypes.JSON,
      this._headers
    );
  ...
  async login(): Promise<IBasicProfile> {
    this._token = await this._authenticator.authenticate(
      'GitHub',
      environment.current.host === 'TEAMS'
    );
    this._setDefaultHeaders(this._token);
  ...
  gists(): Observable<IGist[]> {
    ...
    return this._request.get<IGist[]>(
      url,
      ResponseTypes.JSON,
      this._headers,
      new Date().getTime().toString() /*force bypass of cache*/
    );
  }
  ...
  createOrUpdateGist(
    ...
    if (!(id == null)) {
      url += `/${id}`;
      return this.gist(id).mergeMap(gist => {
  ...
  forkGist(id: string): Observable<IGist> {
    return this._request.post<IGist>(
      `${this._baseUrl}/gists/${id}/forks`,
      undefined,
      ResponseTypes.JSON,
      this._headers
    );
