...
class ApplicationInsights {
  ...
  initialize(instrumentationKey, disable?: boolean) {
    ...
    try {
      ...
      this._current.config.enableDebug = this._current.config.verboseLogging = !environment
        .current.devMode;
  ...
  toggleTelemetry(force?: boolean) {
    ...
    try {
      ...
      Utilities.log(
        new TelemetryError(`Could not toggle telemetry ${force ? 'on' : 'off'}`, e)
      );
    }
  }
      ...
      stop: () => {
        try {
          ...
          if (environment.current.devMode) {
            console.info(name, url, properties, {
              ...measurement,
              duration: (tEnd - tStart) / 1000,
            });
          }
          this._current.trackPageView(name, url, properties, {
            ...measurement,
            duration: (tEnd - tStart) / 1000,
          });
          return tEnd - tStart;
