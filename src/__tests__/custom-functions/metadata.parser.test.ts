...
function parseSampleFile(fileName: string): { description: string; code: string; meta: string } {
    ...
    fs.readdirSync(SAMPLE_DIR).forEach((file: string) => {
        if (file.startsWith('error.')) {
            it(`should throw an error for the function in ${file}`, () => {
                ...
                expect(() => {
                    parseMetadata(source, 'snippetName').forEach((meta) => {
                        if (meta.error) {
                            throw Error();
                        }
                    });
            ...
            it(description, () => {
                expect(parseMetadata(code, 'snippetName')).toEqual(JSON.parse(meta));
            });
        }
    });
});
